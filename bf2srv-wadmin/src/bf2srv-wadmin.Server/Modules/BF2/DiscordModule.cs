...
using bf2srv-wadmin.Server.Commands;
using bf2srv-wadmin.Server.Commands.BF2;
using bf2srv-wadmin.Server.Services;
using bf2srv-wadmin.Shared;
using Discord;
using Discord.WebSocket;
using 4aban.Common.Telemetry;
using MessageType = bf2srv-wadmin.Common.Entities.Game.MessageType;

namespace bf2srv-wadmin.Server.Modules.BF2;
...
!exec```";
    ...
    public async Task SendTextMessageToChannelsAsync(string text, bool debug = false)
    {
        ...
        var channels = debug ? _adminChannels.Where(c => c.Guild.Name.ToLower().Contains("tvbug")) : _adminChannels;
    ...
    public async Task SendEmbedMessageToChannelsAsync(Embed embed, bool debug = false)
    {
        ...
        var channels = debug ? _matchResultChannels.Where(c => c.Guild.Name.ToLower().Contains("tvbug")) : _matchResultChannels;
